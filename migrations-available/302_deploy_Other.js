...
module.exports = (deployer, network, accounts) => {
    ...
    return deployer
        ...
        .then(() => controller.registerContract(accounts[0], 'FD.Funder', false))
        ...
        // Fund Contracts
        .then(() => FlightDelayController
            .deployed()
            // Fund FD.Ledger
            .then(() => log.info('Fund FD.Ledger'))
            .then(() => FlightDelayLedger.deployed())
            .then(FD_LG => FD_LG.sendTransaction({from: accounts[0], value: fund(0.1)}))

            // Fund FD.Underwrite
            .then(() => log.info('Fund FD.Underwrite'))
            .then(() => FlightDelayUnderwrite.deployed())
            .then(FD_UW => FD_UW.sendTransaction({from: accounts[0], value: fund(0.1)}))

            // Fund FD.Payout
            .then(() => log.info('Fund FD.Payout'))
            .then(() => FlightDelayPayout.deployed())
            .then(FD_PY => FD_PY.sendTransaction({from: accounts[0], value: fund(0.1)})))
