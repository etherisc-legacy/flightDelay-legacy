...
module.exports = function (deployer) {
  deployer.deploy(FlightDelayController, { value: web3.toWei(5, 'ether') })
    .then(function () {
    ...
    return deployer.deploy(FlightDelayLedger, FlightDelayController.address, { value: web3.toWei(100, 'ether') });
  }).then(function () {
    return deployer.deploy(FlightDelayNewPolicy, FlightDelayController.address);
  }).then(function () {
    return deployer.deploy(FlightDelayUnderwrite, FlightDelayController.address, { value: web3.toWei(5, 'ether') });
  }).then(function () {
    return deployer.deploy(FlightDelayPayout, FlightDelayController.address, { value: web3.toWei(5, 'ether') });
  }).then(function () {
